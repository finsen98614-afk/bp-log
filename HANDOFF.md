...
```js
{
  ...
  sys:   124,                  // integer 40??60
  dia:   79,                   // integer 20??00
  pulse: 70,                   // integer 25??50, or null
  note:  '?勗憌脣??'          // string capped at 200 chars, or null
}
```
...
8. **Bump the SW cache constant on every deploy.** `const CACHE = 'bp-log-vN'` in `sw.js`. Without a bump the old service worker keeps serving stale files and the update appears not to have worked.
...
- **ca** ??Normal <120/80 繚 Watch 120??29 sys 繚 HTN ??30/80 繚 Treat ??40/90 繚 Crisis ??80/120
- **intl** ??Normal <130/80 繚 High-normal 130??34/80??4 繚 Stage 1 ??35/85 繚 Stage 2 ??50/95 繚 Crisis ??80/120
